...
class SACAgent(BaseAlgorithm):
    def __init__(self, base_name, params):
        ...
        self.bounds_loss_soft_lower_bound: list[float] = self.config.get(
            "bounds_loss_soft_lower_bound", [-1.0]
        )
        self.bounds_loss_soft_upper_bound: list[float] = self.config.get(
            "bounds_loss_soft_upper_bound", [1.0]
        )
        self.bounds_loss_action_dim: list[int] = self.config.get(
            "bounds_loss_action_dim", None
        )

        self.reg_loss_coef = config.get("reg_loss_coef", None)
        self.left_nominal_action_dim: list[int] = self.config.get(
            "left_nominal_action_dim", None
        )
        self.right_nominal_action_dim: list[int] = self.config.get(
            "right_nominal_action_dim", None
        )
    ...
    def update_actor_and_alpha(self, obs, step):
        ...
        actor_loss = (
            torch.max(self.alpha.detach(), self.min_alpha) * log_prob
            - actor_Q
            + b_loss
            + reg_loss
        )
    ...
    def reg_loss(self, mu):
        mu_processed = (
            mu.clone()
        )  # better to use unsquashed mu for numerical stability?
    ...
    def bound_loss(self, mu):
        ...
        with torch.no_grad():
            lb_tensor = (
                torch.tensor(lb, device=self._device, dtype=mu_processed.dtype)
                .unsqueeze(0)
                .repeat(mu_processed.shape[0], 1)
            )
            ub_tensor = (
                torch.tensor(ub, device=self._device, dtype=mu_processed.dtype)
                .unsqueeze(0)
                .repeat(mu_processed.shape[0], 1)
            )

        mu_loss_high = (
            torch.clamp_min(mu_processed[:, action_dim] - ub_tensor, 0.0) ** 2
        )  # violate mu >= ub
        mu_loss_low = (
            torch.clamp_max(mu_processed[:, action_dim] - lb_tensor, 0.0) ** 2
        )  # violate mu <= -lb
        ...
        while True:
            ...
            if self.epoch_num >= self.num_warmup_steps:
                ...
                if self.reg_loss_coef is not None:
                    self.writer.add_scalar(
                        "losses/a_reg_loss",
                        torch_ext.mean_list(reg_losses).item(),
                        self.frame,
                    )

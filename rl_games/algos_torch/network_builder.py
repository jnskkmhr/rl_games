...
class A2CEncoderBuilder(NetworkBuilder):
    """ 
    A2C network with encoder:
    x -> |encoder| -> z -> |actor/critic mlp| -> action, value
                                  |
                                  y
    """
    def __init__(self, **kwargs):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            state_shape = input_shape["state"] # (B, C)
            image_shape = input_shape["image"] # (B, W, H, C)
            
            self.encoder_network_type = self.encoder.get('network_type', 'cnn')
            if self.encoder_network_type == 'cnn':
                encoder_input_shape = image_shape
                cnn_args = {
                    'ctype' : self.encoder['type'], 
                    'input_shape' : encoder_input_shape, 
                    'convs' :self.encoder['convs'], 
                    'activation' : self.encoder['activation'], 
                    'norm_func_name' : self.encoder.get('normalization', None),
                }
                self.actor_encoder = self._build_conv(**cnn_args)
                if self.separate:
                    self.critic_encoder = self._build_conv( **cnn_args)
                encoder_output_shape = self._calc_input_size(encoder_input_shape, self.actor_encoder)
                    
            elif self.encoder_network_type == 'mlp':
                encoder_input_shape = image_shape[0]*image_shape[1]*image_shape[2]
                mlp_args = {
                    'input_size' : encoder_input_shape,
                    'units' : self.encoder['units'], 
                    'activation' : self.encoder['activation'], 
                    'norm_func_name' : self.encoder.get('normalization', None),
                    'dense_func' : torch.nn.Linear,
                    'd2rl' : self.encoder.get('d2rl', False),
                    'norm_only_first_layer' : self.norm_only_first_layer
                }
                self.actor_encoder = self._build_mlp(**mlp_args)
                if self.separate:
                    self.critic_encoder = self._build_mlp(**mlp_args)
                encoder_output_shape = self.encoder['units'][-1]

            encoder_output_size = encoder_output_shape + state_shape[0]
            ...
            if self.has_rnn:
                if not self.is_rnn_before_mlp:
                    rnn_in_size = out_size
                    if self.rnn_concat_input:
                        rnn_in_size += encoder_output_size
            ...
            mlp_init = self.init_factory.create(**self.initializer)
            if self.encoder_network_type == 'cnn':
                cnn_init = self.init_factory.create(**self.encoder['initializer'])
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                a_out = c_out = obs["image"]
                
                if self.encoder_network_type == 'cnn':
                    a_out = self.actor_encoder(a_out)
                    a_out = a_out.contiguous().view(a_out.size(0), -1)
                    c_out = self.actor_encoder(c_out)
                    c_out = c_out.contiguous().view(c_out.size(0), -1)     
                    ...
                    a_out = self.actor_encoder(a_out)
                    c_out = self.critic_encoder(c_out)
                
                # concatenate with propioceptive state
                a_out = torch.cat([a_out, obs["state"]], dim=1)
                c_out = torch.cat([c_out, obs["state"]], dim=1)              
                ...
                if self.encoder_network_type == 'cnn':
                    out = self.actor_encoder(out)
                    out = out.contiguous().view(out.size(0), -1)
                elif self.encoder_network_type == 'mlp':
                    out = out.view(out.size(0), -1)
                    out = self.actor_encoder(out)
                # concatenate with propioceptive state
                out = torch.cat([out, obs["state"]], dim=1)         
    ...
    def build(self, name, **kwargs):
        net = A2CEncoderBuilder.Network(self.params, **kwargs)
        return net
